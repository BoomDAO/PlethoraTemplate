...
namespace PlethoraV2.Utility
{
    ...
    public static class CollectionUtil
    {
        ...
        public static R GetProperty<T, R>(this IEnumerable<T> arr, Predicate<T> match, Func<T, R> func, R defaultValue = default)
        {
            foreach (T item in arr)
            {
                if (match(item)) return func.Invoke(item);
            }
            return defaultValue;
        }
        public static bool TryGetProperty<T, R>(this IEnumerable<T> arr, Predicate<T> match, Func<T, R> func, out R result)
        {
            result = default;
        ...
        public static void Debug<T, DebugValue>(this IEnumerable<T> arr, Func<T, DebugValue> action)
        {
            ...
            foreach (T item in arr)
            {
                $"arr ${arr.GetType().Name}'s element of  index: {index} - {action.Invoke(item)}".Log<UnityEngine.MonoBehaviour>();
                ++index;
            }
        }
        ...
        {
            if (arr.First() == null) return default;
            ...
            int randIndex = rand.Next(0, linkedList.Count);

            return linkedList.ElementAt(randIndex);
        }
        ...
        public static T LocateBest<T>(this IEnumerable<T> arr, Func<T, T, T> match)
        {
            ...
            T target = enumerator.Current;
            enumerator.MoveNext();

            while (enumerator.MoveNext())
            {
                target = match.Invoke(target, enumerator.Current);
            }
        ...
        /// <param name="match"></param>
        /// <returns></returns>
        public static int GetIndex<T>(this IEnumerable<T> arr, Predicate<T> match)
        {
            int index = -1;
        ...
        public static R ElementAt<R>(this LinkedList<R> linkedList, int targetIndex)
        {
            ...
            if (runner == null) return returnVal;
